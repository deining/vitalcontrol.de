...
6. Outro menu de configurações será aberto, no qual você pode salvar vários detalhes sobre o novo bezerro. Use as teclas de seta △ ▽ para selecionar a opção de configuração apropriada. Para as opções de configuração `{{<T "TypeAnimal" >}}`, `{{<T "Sex" >}}`, `{{<T "BirthWeight" >}}`, `{{<T "Breed" >}}`, `{{<T "Whereabouts" >}}`, `{{<T "Multiples" >}}` e `{{<T "CalvingEase" >}}` use as teclas de seta △ ▽ para selecionar a configuração desejada. Para as opções de configuração `{{<T "NationalAnimalID" >}}`, `{{<T "_ID" >}}` e `{{<T "DateBirth" >}}` selecione a opção de configuração correspondente e confirme com `{{<T "Ok" >}}`. Agora use as teclas de seta △ ▽ ◁ ▷ para selecionar os números desejados.
