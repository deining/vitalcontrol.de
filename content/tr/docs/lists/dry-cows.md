...
6. Yeni buzağı hakkında çeşitli detayları kaydedebileceğiniz başka bir ayar menüsü açılır. Uygun ayar seçeneğini seçmek için ok tuşlarını △ ▽ kullanın. `{{<T "TypeAnimal" >}}`, `{{<T "Sex" >}}`, `{{<T "BirthWeight" >}}`, `{{<T "Breed" >}}`, `{{<T "Whereabouts" >}}`, `{{<T "Multiples" >}}` ve `{{<T "CalvingEase" >}}` ayar seçenekleri için istenen ayarı seçmek için ok tuşlarını △ ▽ kullanın. `{{<T "NationalAnimalID" >}}`, `{{<T "_ID" >}}` ve `{{<T "DateBirth" >}}` ayar seçenekleri için ilgili ayar seçeneğini seçin ve `{{<T "Ok" >}}` ile onaylayın. Şimdi istenen rakamları seçmek için ok tuşlarını △ ▽ ◁ ▷ kullanın.
