...
6. Another settings menu opens in which you can save various details about the new calf. Use the arrow keys △ ▽ to select the appropriate setting option. For the setting options `{{<T "TypeAnimal" >}}`, `{{<T "Sex" >}}`, `{{<T "BirthWeight" >}}`, `{{<T "Breed" >}}`, `{{<T "Whereabouts" >}}`, `{{<T "Multiples" >}}` and `{{<T "CalvingEase" >}}` use the arrow keys △ ▽ to select the desired setting. For the setting options `{{<T "NationalAnimalID" >}}`, `{{<T "_ID" >}}` and `{{<T "DateBirth" >}}` select the corresponding setting option and confirm with `{{<T "Ok" >}}`. Now use the arrow keys △ ▽ ◁ ▷ to select the desired numbers.
