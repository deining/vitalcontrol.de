...
6. Otevře se další menu nastavení, ve kterém můžete uložit různé detaily o novém teleti. Pomocí šipek △ ▽ vyberte vhodnou možnost nastavení. Pro možnosti nastavení `{{<T "TypeAnimal" >}}`, `{{<T "Sex" >}}`, `{{<T "BirthWeight" >}}`, `{{<T "Breed" >}}`, `{{<T "Whereabouts" >}}`, `{{<T "Multiples" >}}` a `{{<T "CalvingEase" >}}` použijte šipky △ ▽ k výběru požadovaného nastavení. Pro možnosti nastavení `{{<T "NationalAnimalID" >}}`, `{{<T "_ID" >}}` a `{{<T "DateBirth" >}}` vyberte odpovídající možnost nastavení a potvrďte stisknutím `{{<T "Ok" >}}`. Nyní použijte šipky △ ▽ ◁ ▷ k výběru požadovaných čísel.
